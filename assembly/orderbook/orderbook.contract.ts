...
*/
  ...
  */
  private lockBalance(
    ...
    const balancesTable = new TableStore<BalancesTable>(this.receiver, user);
    if (side == "buy") {
      // ✅ Calculate taker fee (worst case) to lock
      const takerFeeAmount = this.calculateFee(
        total_value.amount,
        pair.taker_fee_bp,
      );
      const takerFee = new Asset(takerFeeAmount, pair.quote_symbol);
      const totalToLock = Asset.add(total_value, takerFee);

      const balance = balancesTable.requireGet(
        pair.quote_symbol.code(),
        "Insufficient quote token",
      );
      check(balance.balance.amount >= totalToLock.amount, "Insufficient funds");

      balance.balance = Asset.sub(balance.balance, totalToLock);
      balance.locked = Asset.add(balance.locked, totalToLock);
      balance.updated_at = currentTimeSec();

      balancesTable.update(balance, this.receiver);
    } else {
      // Seller locks base tokens only (no change needed)
      const balance = balancesTable.requireGet(
        pair.base_symbol.code(),
  ...
  */
  private unlockPartialFill(order: OrdersTable, pair: PairsTable): void {
    ...
    if (order.side != "buy") return; // Only buy orders have quote locked

    const user = order.user;
    const balancesTable = new TableStore<BalancesTable>(this.receiver, user);

    // Calculate what was initially locked (value + taker fee)
    const initialValue = this.calculateTotalValue(
      order.price,
      order.amount,
      pair,
    );
    const initialFee = this.calculateFee(
      initialValue.amount,
      pair.taker_fee_bp,
    );
    const initialLocked = Asset.add(
      initialValue,
      new Asset(initialFee, pair.quote_symbol),
    );

    // Calculate what should still be locked
    const remainingValue = this.calculateTotalValue(
      order.price,
      order.remaining_amount,
      pair,
    );
    const remainingFee = this.calculateFee(
      remainingValue.amount,
      pair.taker_fee_bp,
    );
    const stillNeeded = Asset.add(
      remainingValue,
      new Asset(remainingFee, pair.quote_symbol),
    );
  ...
  private unlockBalance(
    ...
    if (side == "buy") {
      ...
      // ✅ Include the taker fee that was locked
      const takerFeeAmount = this.calculateFee(
        locked_value.amount,
        pair.taker_fee_bp,
      );
      ...
      // Seller - no change needed (base tokens only)
      const balance = balancesTable.requireGet(
        pair.base_symbol.code(),
        "Balance error",
      );
  ...
  */
  ...
  private executeTrade(
    ...
    const maxFeeAmount = this.calculateFee(
      totalValue.amount,
      pair.taker_fee_bp,
    );
    const maxFee = new Asset(maxFeeAmount, pair.quote_symbol);
    const totalLocked = Asset.add(totalValue, maxFee);

    // ✅ Calculate what is actually needed
    const actualNeeded = Asset.add(totalValue, buyerFee);

    // ✅ Calculate refund (if buyer is maker, they pay less)
    const refundAmount = totalLocked.amount - actualNeeded.amount;
    const refund = new Asset(refundAmount, pair.quote_symbol);
    ...
    // Deduct total locked amount
    check(
      buyerQuote.locked.amount >= totalLocked.amount,
      "Buyer insufficient locked",
    );

    buyerQuote.locked = Asset.sub(buyerQuote.locked, totalLocked);
    ...
    this.transfer(
      ...
      buyer,
      tradeAmount,
      `Order #${buyOrder.order_id}: bought ${tradeAmount.toString()} @ ${executionPrice.toString()}`,
    );

    // ✅ Send quote proceeds to seller (after deducting fee)
    ...
    this.transfer(
      ...
      seller,
      sellerNetProceeds,
      `Order #${sellOrder.order_id}: sold ${tradeAmount.toString()} @ ${executionPrice.toString()}`,
    );

    // ✅ Send fees to fee_recipient
    const config = this.getConfig();
    ...
    // ✅ IMMEDIATE REFUND: Send excess fee back to buyer's wallet
    if (refund.amount > 0) {
      this.transfer(
        pair.quote_contract,
        this.receiver,
  ...
  */
  private executeMarketTrade(
    ...
    if (marketSide == "buy") {
      // Buyer is taker (market order) - no refund needed
      const buyerQuote = buyerBalances.requireGet(
        pair.quote_symbol.code(),
        "Buyer balance error",
      );
      ...
      check(
        buyerQuote.locked.amount >= buyerTotalNeeded.amount,
        "Buyer insufficient locked",
      );
      ...
      // Seller is maker - calculate refund
      const sellerBase = sellerBalances.requireGet(
        pair.base_symbol.code(),
        "Seller balance error",
      );
      ...
      // Buyer is maker - they locked taker fee, pay maker fee, get refund
      const buyerQuote = buyerBalances.requireGet(
        pair.quote_symbol.code(),
        "Buyer balance error",
      );

      // What was locked (value + taker fee)
      const maxFeeAmount = this.calculateFee(
        tradeQuote.amount,
        pair.taker_fee_bp,
      );
      const maxFee = new Asset(maxFeeAmount, pair.quote_symbol);
      const totalLocked = Asset.add(tradeQuote, maxFee);

      // What is actually needed (value + maker fee)
      const actualNeeded = Asset.add(tradeQuote, makerFee);
      ...
      // ✅ IMMEDIATE REFUND to buyer (they were maker)
      if (refund.amount > 0) {
        this.transfer(
          pair.quote_contract,
          this.receiver,
  ...
  */
  private executeStopLossTrade(
    ...
    if (stopLossOrder.side == "buy") {
      // =========================
      // STOP-LOSS BUY (buyer = taker, seller = maker)
      // =========================

      // Buyer (taker) pays totalValue + takerFee
      const buyerQuote = buyerBalances.requireGet(
        pair.quote_symbol.code(),
        "Buyer balance error",
      );

      const buyerTotalNeeded = Asset.add(totalValue, takerFee);
      ...
      // Seller (maker) delivers base, receives totalValue - makerFee
      const sellerBase = sellerBalances.requireGet(
        pair.base_symbol.code(),
        "Seller balance error",
      );
      ...
      this.transfer(
        ...
        buyer,
        tradeAmount,
        `Stop-loss #${stopLossOrder.order_id}: ${tradeAmount.toString()} @ ${executionPrice.toString()}`,
      );

      const sellerNet = Asset.sub(totalValue, makerFee);
      this.transfer(
        pair.quote_contract,
        this.receiver,
        seller,
        sellerNet,
        `Order #${limitOrder.order_id}: sold ${tradeAmount.toString()}`,
      );
    } else {
      // =========================
      // STOP-LOSS SELL (seller = taker, buyer = maker)
      // =========================

      // Seller (taker) delivers base
      const sellerBase = sellerBalances.requireGet(
        pair.base_symbol.code(),
        "Seller balance error",
      );
      ...
      // Buyer (maker) locked totalValue + takerFee but should pay totalValue + makerFee
      const buyerQuote = buyerBalances.requireGet(
        pair.quote_symbol.code(),
        "Buyer balance error",
      );

      const totalLocked = Asset.add(totalValue, takerFee);

      const actualNeeded = Asset.add(totalValue, makerFee);
      const refundAmount = totalLocked.amount - actualNeeded.amount;
      const refund = new Asset(refundAmount, pair.quote_symbol);
      ...
      this.transfer(
        ...
        buyer,
        tradeAmount,
        `Order #${limitOrder.order_id}: bought ${tradeAmount.toString()}`,
      );

      const sellerNet = Asset.sub(totalValue, takerFee);
      this.transfer(
        pair.quote_contract,
        this.receiver,
        seller,
    ...
    // Trade record (FIXED)
    // =========================
    const buyerFee = stopLossOrder.side == "buy" ? takerFee : makerFee;

    const sellerFee = stopLossOrder.side == "buy" ? makerFee : takerFee;
    ...
    this.transfer(
      ...
      config.fee_recipient,
      totalFees,
      `Fees from stop-loss on pair ${pair.pair_id}`,
    );
